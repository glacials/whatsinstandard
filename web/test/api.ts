        ...
        describe(set.name, () => {
          ...
          describe("enter date", () => {
            ...
            it("should be an ISO 8601 datetime if present", () => {
              expect(
                Date.parse(enterDate || "1970-01-01T00:00:00.000")
              ).to.be.a("number");
            });

            it("should represent midnight if present", () => {
              expect(enterDate || "1970-01-01T00:00:00.000").to.contain(
                "00:00:00.00"
              );
            });
          ...
          describe("exit date", () => {
            ...
            it("should be an ISO 8601 datetime if present", () => {
              expect(Date.parse(exitDate || "1970-01-01T00:00:00.000")).to.be.a(
                "number"
              );
            });

            it("should represent midnight if present", () => {
              expect(exitDate || "1970-01-01T00:00:00.000").to.contain(
                "00:00:00.00"
              );
            });
      ...
      body.bans.forEach((ban: any) => {
        describe(ban.card_name, () => {
          ...
          describe("card image URL", () => {
            ...
            it("should use HTTPS", () => {
              expect(new URL(cardImageURL).protocol).to.eq("https:");
            });
          });
          ...
          describe("announcement URL", () => {
            ...
            it("should use HTTPS", () => {
              expect(new URL(announcementURL).protocol).to.eq("https:");
            });

            it("should be an official Wizards of the Coast link", () => {
              expect(new URL(announcementURL).hostname).to.eq(
                "magic.wizards.com"
              );
            });
          });
